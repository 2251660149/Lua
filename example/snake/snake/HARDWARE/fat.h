...
typedef __packed struct 
{
		...
		WORD	bpbRootDirEnts;	// number of root directory entriesg根目录最多容纳目录项fat32 不使用
		WORD	bpbSectors;		// total number of sectors 最大32M 2GB卡不适用该处
		BYTE	bpbMedia;		// media descriptor 0Xf8 本地硬盘
		WORD	bpbFATsecs;		// number of sectors per FAT FAT32 此处置0
		WORD	bpbSecPerTrack;	// sectors per track 磁道
		WORD	bpbHeads;		// number of heads 磁头数
		DWORD	bpbHiddenSecs;	// # of hidden sectors 分区前已使用的扇区 扇区偏移量
// 3.3 compat ends here
		DWORD	bpbHugeSectors;	// # of sectors if bpbSectors == 0 文件系统扇区数
// 5.0 compat ends here
		DWORD     bpbBigFATsecs;// like bpbFATsecs for FAT32 每个FAT表大小扇区
		WORD      bpbExtFlags;	// extended flags:
#define FATNUM    0xf			// mask for numbering active FAT
#define FATMIRROR 0x80			// FAT is mirrored (like it always was)
		WORD      bpbFSVers;	// filesystem version                                版本号
#define FSVERS    0				// currently only 0 is understood
		DWORD     bpbRootClust;	// start cluster for root directory          根目录簇号 通常是2
		WORD      bpbFSInfo;	// filesystem info structure sector        文件系统信息扇区
		WORD      bpbBackup;	// backup boot sector  备份引导扇区位置
