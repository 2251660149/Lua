...
//13,lrc;14,txt;15,c;16,h;17,file;18,FON;19,SYS;20,bmp;21,jpg;22,jpeg;23,bin  
const unsigned char *filetype[24]=
{
"MP1","MP2","MP3","MP4","M4A","3GP","3G2","OGG","ACC","WMA","WAV","MID","FLA",
"LRC","TXT","C  ","H  ","   ","FON","SYS","BMP","JPG","JPE","lua"
};
//返回扩展名类型
//输入:exName 文件扩展名
...
{	
	...
	//LCD_ShowString(20,100,"read success");
	return 1;//读取成功
} 
...
FileInfoStruct F_Search(u32 cluster,unsigned char *Name,u32 type)
{
	...
	}else//其他文件夹/FAT32系统
	{
		...
		{
			sector=fatClustToSect(tempclust);
			for(cnt=0;cnt<SectorsPerClust;cnt++)//遍历8扇区
			{
				if(SD_ReadDisk(fat_buffer,sector+cnt,1)) //将1个扇区512b 装入fat_buffer
				{
					FileInfo.F_StartCluster=0;//读数错误 
					return FileInfo;   
				}
				for(offset=0;offset<512;offset+=32)// 一个目录32个b描述
				{
					item=(direntry *)(&fat_buffer[offset]); //存入文件结构体 32b
