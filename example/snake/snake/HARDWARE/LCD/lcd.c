...
{   			    
    ...
    {   
        ...
        for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,color);//画实心点
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size,0,YELLOW);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,0,YELLOW); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
//}
#include "string.h"
void LCD_ShowString(u16 x,u16 y,u8 size,const char *p,u16 color)
{         
	u16 x0=x;
//	width+=x;
//	height+=y;
	u16 width=strlen(p)*(size);
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
    {       
        if(x>=240){x=0;y+=size;}
    //    if(y>=height)break;//退出
